...
## Game UUID（平台 session_id 锚点）

**每个游戏一个永久 UUID**，作为平台 Aigram 所有 session-scoped 接口（排行榜、存档、事件、统计）的 `session_id`。

**三条铁律：**

1. **真随机生成**——必须用 `uuid.uuid4()`（122-bit），不能手编、不能用 game id 拼字符串
2. **一旦生成，永不更新**——游戏代码迭代、重新部署、改名、换 repo 都不变 UUID
3. **删了的游戏 UUID 永不复用**——`uuid4` 的随机性自然保证（碰撞概率 ≈ 0），新游戏一律走 `uuid4` 新生成，不要"回收"

**接入新游戏的标准流程：**

```bash
# 1. 把游戏 id 加进 games.json（暂不填 uuid 字段）
# 2. 跑脚本：幂等地为缺失的条目补 uuid4
python3 /Users/yin/code/games/games/add-uuids.py

# 3. 跑脚本：把每个游戏的 uuid 注入到本地源码
#    - React/Vite 游戏 → 写 src/game-id.ts (window.__GAME_UUID__)
#    - vanilla HTML 游戏 → 写 <meta name="game-uuid"> 到 index.html
python3 /Users/yin/code/games/scripts/sync-game-ids.py
```

两个脚本**都幂等**——已有 UUID 不会被改写。重跑没副作用。

**完整的 runtime / 排行榜 / 存档 / 生图接入**：见 `/Users/yin/code/games/RUNTIME.md` 和 `.claude/skills/{aigram-api, game-persistence, gen-image}/SKILL.md`。
